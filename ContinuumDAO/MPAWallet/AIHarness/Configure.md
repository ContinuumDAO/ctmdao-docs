...
### 3. Variables, MCP servers, and default web browser / search
...
5. If you use research or plan flows that need the web, configure a **default web browser / search** MCP (and any matching Variables) so the agent has a working search path.

Pattern: enable the MCP row → set the matching Variable names → confirm they show as configured.
...
2. Put **`TELEGRAM_BOT_TOKEN`**, the webhook secret, and **`NGROK_AUTHTOKEN`** in **Variables** as the UI requires.
