...
- **Direct web3 protocol access** — the node app and optional AI agent connect **directly** to major DeFi protocols (Uniswap, Aave, Curve, Aerodrome, Compound, GMX, Hyperliquid, Derive, Lido, and others) for **secure Private-Key-less trading** — swaps, lending, staking, perps, options, and bridges — always through your MPC KeyGen and the same [Accept/Reject loop](/ContinuumDAO/MPAWallet/MPCAcceptRejectLoop.md). Full list: [DeFi protocol support](/ContinuumDAO/MPAWallet/DeFiProtocolSupport.md).
...
Direct **web3 / DeFi protocol** integrations built into the node (no separate browser wallet or key export): [DeFi protocol support](/ContinuumDAO/MPAWallet/DeFiProtocolSupport.md) — Uniswap, Aave, Curve, Aerodrome, Compound, GMX, Hyperliquid, Derive, and other major protocols for **Private-Key-less trading** from the node app or AI agent MCP. Interactive candlesticks, analysis, and trade ideas: [AI charting](/ContinuumDAO/MPAWallet/AICharting.md), [Technical analysis](/ContinuumDAO/MPAWallet/TechnicalAnalysis.md), and [Trade ideas](/ContinuumDAO/MPAWallet/TradeIdeas.md).
...
MPA wallet access can be paid via a **monthly subscription**, or — for node operators who **attach veCTM to their node** — through **subscription through staking**: **free use of the wallet up to a generous governance-set free signature limit** for KeyGens registered in that **Group**. Limits and any overage fees are set by DAO voting. The same qualifying attach also unlocks **[Private VPN](/ContinuumDAO/PrivateVPN.md)** on the **node** — a veCTM privilege (not a separate Linea subscription). Privilege follows the node’s recorded attach groups, not “whoever is withdraw authority right now.”
