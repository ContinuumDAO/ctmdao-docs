...
The same node software serves **two roles**: (1) **MPA wallets** — decentralized custody without a full Private Key, with optional AI and a human-in-the-loop Accept circuit breaker (simplest **2/2**); (2) **cross-chain Continuum signing** — larger Groups (ideally **5+ independent** operators, **3/5 TSS**) that secure messaging between chains. Details: [MPA wallet Overview](/ContinuumDAO/MPAWallet/Overview.md). The Continuum allows all EVMs and almost all non-EVMs to connect.
